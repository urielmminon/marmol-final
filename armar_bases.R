...
summary(walmartSpread$prod_distintos)
...
summary(walmartSpread$prod_devueltos)
...
walmart.testDev <- walmart.test %>% group_by(VisitNumber) %>% 
  summarise(min_p = min(ScanCount)) 
nrow(walmart.testDev)
head(walmart.testDev)
...
summary(walmart.testSpread$prod_distintos)
...
summary(walmart.testSpread$prod_devueltos)
